  ...
  launchUpdatesListener() {
    ...
    try {
      ...
      this._eventListener.on('message', (event) => {
        ...
        Homey.app.debug(Homey.app.api.getNvrName() + ': Received new websocket message');
      });
  ...
  configureUpdatesListener(driver) {
    ...
    this._eventListener.on('message', (event) => {
      ...
      if (Homey.env.DEBUG === 'true') {
        Homey.app.debug('Websocket getting new event from ' + device.getName());
        Homey.app.debug(payload);
        Homey.app.debug('---- end websocket event ----');
      }
