...
class UfPWebsocket {
  ...
  launchUpdatesListener() {
    ...
    Homey.app.log('Update listener: ' + this.updatesUrl() + '?' + params.toString());

    try {
      ManagerApi.realtime(UfvConstants.EVENT_SETTINGS_WEBSOCKET_STATUS, 'Connecting');
      Settings.set(UfvConstants.EVENT_SETTINGS_WEBSOCKET_STATUS, 'Connecting');
      ...
      const pingpong = setInterval(() => {
        if (this._eventListener !== null) {
          ...
          Homey.app.log(Homey.app.api.getNvrName() + ': Send ping to websocket.');
        }
      }, 15000);
      ...
      this._eventListener.on('pong', (event) => {
        ...
        let lastPong = new Date().toString();
        ManagerApi.realtime(UfvConstants.EVENT_SETTINGS_WEBSOCKET_LASTPONG, lastPong);
        Settings.set(UfvConstants.EVENT_SETTINGS_WEBSOCKET_LASTPONG, lastPong);
        Homey.app.log(Homey.app.api.getNvrName() + ': Received pong from websocket.');
      });

      // Received ping
      this._eventListener.on('message', (event) => {
        // update lastPong variable
        let lastMessage = new Date().toString();
        ManagerApi.realtime(UfvConstants.EVENT_SETTINGS_WEBSOCKET_LASTMESSAGE, lastMessage);
        Settings.set(UfvConstants.EVENT_SETTINGS_WEBSOCKET_LASTMESSAGE, lastMessage);
        Homey.app.log(Homey.app.api.getNvrName() + ': Received new websocket message');
      });

      // Connection opened
      this._eventListener.on('open', (event) => {
        Homey.app.log(Homey.app.api.getNvrName() + ': Connected to the UniFi realtime update events API.');
        ManagerApi.realtime(UfvConstants.EVENT_SETTINGS_WEBSOCKET_STATUS, 'Connected');
        Settings.set(UfvConstants.EVENT_SETTINGS_WEBSOCKET_STATUS, 'Connected');
      });
      ...
      this._eventListener.on('close', () => {
        ...
        Settings.set(UfvConstants.EVENT_SETTINGS_WEBSOCKET_STATUS, 'Disconnected');
      ...
      this._eventListener.on('error', (error) => {
        ...
        if (error.message !== 'WebSocket was closed before the connection was established') {
          Homey.app.log(Homey.app.api.getHost(), +': ' + error);
        }

        ManagerApi.realtime(UfvConstants.EVENT_SETTINGS_WEBSOCKET_STATUS, error.message);
        Settings.set(UfvConstants.EVENT_SETTINGS_WEBSOCKET_STATUS, error.message);
      ...
      Homey.app.log(Homey.app.api.getNvrName() + ': Error connecting to the realtime update events API: ' + error);
      ManagerApi.realtime(UfvConstants.EVENT_SETTINGS_WEBSOCKET_STATUS, error);
      Settings.set(UfvConstants.EVENT_SETTINGS_WEBSOCKET_STATUS, error);
    }
  ...
  configureUpdatesListener(driver) {
    ...
    this._eventListener.on('message', (event) => {
      ...
      if (!updatePacket) {
        Homey.app.log(Homey.app.api.getNvrName() + ': Unable to process message from the realtime update events API.');
        return;
      }
      ...
      Homey.app.log('Websocket getting new event from ' + device.getName());
      Homey.app.log(payload);
      Homey.app.log('---- end websocket event ----');
