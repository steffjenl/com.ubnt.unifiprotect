...
class UfPapi {
  ...
  loginProxy(host, username, password) {
    ...
    return new Promise((resolve, reject) => {
      ...
      Settings.set(UfvConstants.EVENT_SETTINGS_STATUS, 'Connecting');
      ...
      const req = https.request(options, res => {
        ...
        res.on('end', () => {
          ...
          Settings.set(UfvConstants.EVENT_SETTINGS_STATUS, 'Connected');
          //
      ...
      req.on('error', error => {
        ...
        Settings.set(UfvConstants.EVENT_SETTINGS_STATUS, 'Disconnected');
        return reject(error);
      });
