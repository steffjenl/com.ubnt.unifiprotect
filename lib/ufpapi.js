...
class UfPapi {
  ...
  //     };
  //
  //     resolve(`https://${this.getHost()}:${this._nvrPort}${UFV_API_ENDPOINT}/thumbnails/${thumbnailId}${this._toQueryString(params)}`);
  //   });
  // }
  //
  ...
  getSnapShotUrl(camera, widthInPixels = 1920) {
    return new Promise((resolve, reject) => {
      ...
      resolve(`https://${this.getHost()}:${this._nvrPort}${UFV_API_ENDPOINT}/cameras/${camera.id}/snapshot${this._toQueryString(params)}`);
    });
  }
  ...
  loginProxy(host, username, password) {
    console.log('Logging in with proxy.');
    ...
    return new Promise((resolve, reject) => {
      ...
      this.getBootstrapInfo()
        .then(() => {
          ...
          resolve(`https://${this._host}:${this._nvrPort}${UFV_API_ENDPOINT}/cameras/${camera.id}/snapshot${this._toQueryString(params)}`);
        })
        .catch(error => console.log(error));
    });
  }
  ...
  getMotionEvents() {
    return new Promise((resolve, reject) => {
      ...
      const startTime = (this._lastMotionAt == null ? start.getTime() : this._lastMotionAt);
